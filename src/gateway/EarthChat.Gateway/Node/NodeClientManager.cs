...
public class NodeClientManager
{
    ...
    public void Remove(string service, string key)
    {
        ...
        if (nodeClients.IsEmpty) _nodeClients.TryRemove(service.ToLower(), out _);
    }
    ...
    public void Update(string service, string key, NodeClientStats status)
    {
        ...
        if (nodeClients.TryGetValue(key.ToLower(), out var nodeClient)) nodeClient.Stats = status;
    }
    ...
    public IEnumerable<(string key,List<NodeClient> clients)> GetAllWithService()
    {
        return _nodeClients.Select(x => (x.Key, x.Value.Values.ToList()));
    }
